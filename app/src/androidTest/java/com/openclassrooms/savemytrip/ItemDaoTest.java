...
public class ItemDaoTest {
    ...
    private static User USER_DEMO = new User(USER_ID, "Philippe", "https://wwww.google.fr, ");

    private static Item NEW_ITEM_PLACE_TO_VISIT = new Item("Visite cet endroid de rêve", 0, USER_ID);
    private static Item NEW_ITEM_IDEA = new Item("ON pourrait faire du chien de traîneau", 1, USER_ID);
    private static Item NEW_ITEM_RESTAURANTS = new Item("Visite cet endroid de rêve", 2, USER_ID);
    ...
    public void insertAndUpdate() throws InterruptedException {
        // BEFORE : Adding demo user & items. Next , update item added & re-save it
        this.database.userDao().createUser(USER_DEMO);
        this.database.itemDao().insertItem(NEW_ITEM_PLACE_TO_VISIT);
